...
function createDinosaurCard(dinosaur) {
	...
	number.classList.add(`group${dinosaur.group_letter}`);
	number.textContent = `${dinosaur.group_letter}${dinosaur.card_number}`;
	card.appendChild(number);

	const title = document.createElement("dinoname");
	title.textContent = `${dinosaur.name}`;
	card.appendChild(title);

	const image = document.createElement("img");
	// image.classList.add("dinoimg");
	image.src = `images/${dinosaur.name}.jpg`;
	card.appendChild(image);

	const infoList = document.createElement("table");

	const footprintContainer = dinoInfo(
		`images/footprint.png`,
		dinosaur.footprint,
		dinosaur.group_letter
	);
	infoList.appendChild(footprintContainer);

	const yearsContainer = dinoInfo(
		`images/years.png`,
		dinosaur.years,
		dinosaur.group_letter
	);
	infoList.appendChild(yearsContainer);

	const iqContainer = dinoInfo(
		`images/iq.png`,
		dinosaur.iq,
		dinosaur.group_letter
	);
	infoList.appendChild(iqContainer);

	const heightContainer = dinoInfo(
		`images/height.png`,
		dinosaur.height,
		dinosaur.group_letter
	);
	infoList.appendChild(heightContainer);

	const EggSizeContainer = dinoInfo(
		`images/egg_size.png`,
		dinosaur.egg_size,
		dinosaur.group_letter
	);
	infoList.appendChild(EggSizeContainer);

	const ToothSizeContainer = dinoInfo(
		`images/tooth_size.png`,
		dinosaur.tooth_size,
		dinosaur.group_letter
	);
	infoList.appendChild(ToothSizeContainer);

	card.appendChild(infoList);

	document.getElementById("dinosaurCards").appendChild(card);
}

function dinoInfo(source, text, groupLetter) {
	...
	info.classList.add(`group${groupLetter}`);
	info.textContent = text;
	container.appendChild(info);
	return container;
}

function sortDinosaurs(sortBy, buttonName) {
	const dinosaurCards = document.getElementById("dinosaurCards");
	const sortedDinosaurs = dinosaurs.slice().sort((a, b) => {
		return a[sortBy] - b[sortBy];
	});

	// Clear existing cards
	dinosaurCards.innerHTML = "";

	// Create cards for sorted dinosaurs
	sortedDinosaurs.forEach(createDinosaurCard);

	const buttons = document.getElementsByTagName("button");
	Object.values(buttons).forEach((button) => {
		button.classList.remove("selected", "circle");
	});
	const button = document.getElementById(buttonName);
	button.classList.add("selected", "circle");
}
...
// Loop through the dinosaurs and create cards
dinosaurs.forEach(createDinosaurCard);
